...
const PlanDurationSwitcher = tw.div`block w-full max-w-xs sm:inline-block sm:w-auto border-2 rounded-full px-1 py-1 m-8`;
        ...
        <HeaderContainer>
          ...
          <PlanDurationSwitcher>
            {planDurations.map((planDuration, index) => (
              <SwitchButton
                active={
                  (isLogin && planDuration.switcherText === "Log In") ||
                  (!isLogin && planDuration.switcherText === "Sign Up")
                }
                onClick={() => {
                  planDuration.switcherText === "Log In"
                    ? setStatus(true)
                    : setStatus(false);
                }}
              >
