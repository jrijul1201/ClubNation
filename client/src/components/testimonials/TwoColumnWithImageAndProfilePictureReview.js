...
const ControlContainer = tw.div`absolute top-0 right-auto bg-gray-100 px-6 py-4 rounded-3xl border`;
const ControlButton = styled(PrimaryButton)`
  ${tw`mx-3 rounded-md text-gray-100 p-2`}
  svg {
    ${tw`w-5 h-5`}
  }
`;
              ...
              <TestimonialImageSlider arrows={false} ref={setImageSliderRef} asNavFor={textSliderRef} fade={true}>
                {testimonials.map((testimonial, index) => (
                  <ImageAndControlContainer key={index}>
                    ...
                    <ControlContainer>
                      <ControlButton onClick={() => {setStatus(false)}}>
                        {/* <ChevronLeftIcon /> */}
                        Sign Up
                      </ControlButton>
                      <ControlButton onClick={() => {setStatus(true)}}>
                        {/* <ChevronRightIcon /> */}
                        Log In
                      </ControlButton>
                    </ControlContainer>
